...
MC_Ed %>% gather(key="depth",value="irrad",-'wv') %>% 
  mutate(quanta = (irrad / 1e6) * avogadro) -> mced
...
mced %>% filter(wv <= 400) %>%
  ...
  summarise(umol = signif(sum(irrad),2),
            quanta = signif(sum(irrad/1e6),3)*avogadro
            ) -> umol
qplot(data=umol,depth,umol,main='Micromoles of quanta above 400 nm at increasing depth (m)') + theme_classic()
```
...
The irradiance should be ~ 0.03 micromoles of violet - green light.
